...
describe('services/redemptions.service', () => {
  ...
  test('redeemCredit throws if purchase missing', async () => {
    ...
    await expect(redeemCredit({ purchaseId: 'x', customerId: 'c1' })).rejects.toThrow(/not found/i);
  });
  ...
  test('redeemCredit throws if creditsLeft is 0', async () => {
    const purchaseZero = {
      ...
      creditsLeft: 0,
      save: jest.fn(),
      session: () => Promise.resolve({ _id: 'p1', creditsLeft: 0 }),
    };
    Purchase.findOne.mockReturnValue(purchaseZero);
    await expect(redeemCredit({ purchaseId: 'p1', customerId: 'c1' })).rejects.toThrow(
      /No credits left/i
    );
  });
});
